...
@Component({
  ...
  selector: 'app-startendexamService',
...
export class StartendexamComponent implements OnInit {
  ...
  constructor(private startendexamService: StartendexamService,
              private router:Router,private submissionService:SubmissionService,
  ...
  getExamById(examId:number){
    ...
    this.startendexamService.getExamById(examId).subscribe(data =>{
  ...
  getSubmissionId = () => {
    if(sessionStorage.getItem("submissionId") != '0'){
